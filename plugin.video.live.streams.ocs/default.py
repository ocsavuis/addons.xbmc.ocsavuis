...
def getSources():
        ...
        if addon.getSetting("browse_community") == "true":
            addDir('Community Files','community_files',16,icon,FANART,'','','','',True)
        if os.path.exists(source_file)==True:
            ...
            try:
                if len(url) > 1:
                    ...
                    if "RunPlugin" in url[0]:
                    	...
                    	print url[0],name,thumbnail,fanArt
                    	addDir(name,url[0],18,thumbnail,fanArt,'','','','','')
                    else:
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False):
        ...
        ok=True
        isfolderpar = True
        if "RunPlugin" in url: isfolderpar=False
        if date == '':
            date = None
        else:
            description += '\n\nDate: %s' %date
        ...
        ok=xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]),url=u,listitem=liz,isFolder=isfolderpar)
        return ok
...
if mode==None:
    ...
    xbmc.executebuiltin(url.replace(";",""))
